...
If `UI_PR_NUMBER` is omitted, look up the latest open PR on the consumer repo automatically.
...
# Consumer UI repo (where UI PRs come from)
UI_OWNER="innocito"
UI_REPO="consumer"
...
```
mcp__playwright__browser_navigate(url="https://drivejoulez.com")
...
python dashboard/utils/client.py event --type log --stage inspect_dom \
  --message "Navigating to https://drivejoulez.com…" 2>/dev/null || true
python dashboard/utils/client.py event --type log --stage inspect_dom \
...
Use the **Edit tool** to apply each locator replacement directly in `pages/booking_page.py`.
...
---
*Auto-generated by the Joulez Self-Heal Agent*
```
